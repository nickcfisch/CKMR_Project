//Stock Catch at age assessment with the inclusion of CKMR data, in this case Parent-offspring Pairs
...
//POPs 
  ...
  //So the exp reproductive output of the parent in the year of offsprings birth / total reprod output that year
   if(born_year_young(i)>0){
	POP_prob(i) = 4*(Mat(age_diff(i)-1)*Wtage(age_diff(i)-1) / spbiomass(born_year_young(i)-1));
   }    
   if(born_year_young(i)<1){
	POP_prob(i) = 4*(Mat(age_diff(i)-1)*Wtage(age_diff(i)-1) / SSB0);
   }
  }
