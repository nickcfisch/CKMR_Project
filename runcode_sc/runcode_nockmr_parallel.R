...
# saveRDS(res_list_Cod, file=paste0(wd,"/results/SCAAfit_Cod_N5000_Ind50_20yrs_noCKMR.RData"))
# res_list_Flatfish  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Flatfish_wdat[[s]], sd_ind=0.50)}
# saveRDS(res_list_Flatfish, file=paste0(wd,"/results/SCAAfit_Flatfish_N5000_Ind50_20yrs_noCKMR.RData"))
 res_list_Sardine  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Sardine_wdat[[s]], sd_ind=0.50)}
 saveRDS(res_list_Sardine, file=paste0(wd,"/results/SCAAfit_Sardine_N5000_Ind50_20yrs_noCKMR.RData"))

# res_list_Cod<-res_list_Flatfish<-res_list_Sardine<-list()
# res_list_Cod  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Cod_wdat[[s]], sd_ind=0.25)}
# saveRDS(res_list_Cod, file=paste0(wd,"/results/SCAAfit_Cod_N5000_Ind25_20yrs_noCKMR.RData"))
# res_list_Flatfish  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Flatfish_wdat[[s]], sd_ind=0.25)}
# saveRDS(res_list_Flatfish, file=paste0(wd,"/results/SCAAfit_Flatfish_N5000_Ind25_20yrs_noCKMR.RData"))
# res_list_Sardine  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Sardine_wdat[[s]], sd_ind=0.25)}
# saveRDS(res_list_Sardine, file=paste0(wd,"/results/SCAAfit_Sardine_N1000_Ind25_20yrs_noCKMR.RData"))
