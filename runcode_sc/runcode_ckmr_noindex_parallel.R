...
#N_sim<-c( 5, 15 ,22, 28, 33, 49, 52 ,58)
#res_list_Cod  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Cod_wdat[[s]])}
#saveRDS(res_list_Cod, file=paste0(wd,"/results/SCAAfit_Cod_N1000_NoIndex_ckmrmultinom20_9.RData"))

N_sim<-c(1:10)
res_list_Flatfish  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Flatfish_wdat[[s]])}
saveRDS(res_list_Flatfish, file=paste0(wd,"/results/SCAAfit_Flatfish_N5000_NoIndex_ckmrmultinom20_1_10.RData"))

#N_sim<-c(3 , 4  ,9 ,11, 12 ,14 ,26 ,27 ,29, 30 ,31 ,37 ,39, 40, 46 ,48, 49, 58, 60, 62, 64, 68, 70, 71, 72 ,73, 82, 88, 90 ,94, 96, 97)
#res_list_Sardine  <- foreach (s=N_sim, .packages=c("TMB")) %dopar%{run(s=s, OM=Sardine_wdat[[s]])}
#saveRDS(res_list_Sardine, file=paste0(wd,"/results/SCAAfit_Sardine_N5000_NoIndex_ckmrmultinom20_2.RData"))
