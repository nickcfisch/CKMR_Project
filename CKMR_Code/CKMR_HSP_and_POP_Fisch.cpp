...
//CKMR objects
  ...
  for(i=0;i<=age_diff_hsp.size()-1;i++){
   matrix<Type> m1(age_diff_hsp(i),lage+age_diff_hsp(i)-1);
   surv_prob_aa(i) = m1;
  }
...
// - 1s are for TMB indexing which starts at zero
  ...
  //This fills in the survival in the year of first born, Don't know how to fill in a segment of a matrix
   ...
   for(j=0;j<=age_diff_hsp(i)-1-1;j++){  // j is age of the potential parent at the year of the first born, which needs to be integrated over
    for(k=0;k<=lage+age_diff_hsp(i)-2-1;k++){
...
// So parent has to survive year of the birth, year after the birth,.. up to age difference
	...
	if(k>lage-1){
     if(born_year_old_hsp(i)>0){
      surv_prob_aa(i)(j+1,k+1)= surv_prob_aa(i)(j,k) * S(born_year_old_hsp(i)+j,lage-1);
	 }
     if(born_year_old_hsp(i)<1){
      surv_prob_aa(i)(j+1,k+1)= surv_prob_aa(i)(j,k) * exp(-1*NatMort);
     }
   ...
   surv_prob.row(i) = surv_prob_aa(i).block(age_diff_hsp(i)-1,age_diff_hsp(i)-1,1,lage); //getting subset of matrix, starting at (x1,y1), and taking 1 row of 15 columns  
...
//CKMR_data(i,1) is the year of second born, and CKMR_data(i,2) is the age difference
   for(j=0;j<=lage-1;j++){  
    if((j+age_diff_hsp(i))<=(lage-1)){          //If we are not in the plus group
    ...
    if((j+age_diff_hsp(i))>(lage-1)){            //If we are in the plus group
     if(born_year_old_hsp(i)>0){                //if we're not in unfished years
      HSP_prob_aa(i,j) = ( (N(born_year_old_hsp(i)-1,j)*Mat(j)*Wtage(j)) / spbiomass(born_year_old_hsp(i)-1) ) * surv_prob(i,j)  * (4 * Mat(lage-1)*Wtage(lage-1) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));	
	 }
	 if(born_year_old_hsp(i)<1){                //if we are in unfished years
	  if(born_year_old_hsp(i)+age_diff_hsp(i)>0){   //if second born is not in unfished years
       HSP_prob_aa(i,j) = ( (N0_age(j)*Mat(j)*Wtage(j)) / SSB0 ) * surv_prob(i,j)  * (4 * Mat(lage-1)*Wtage(lage-1) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));	
	  }
  	  if(born_year_old_hsp(i)+age_diff_hsp(i)<1){   //if second born is in unfished years
