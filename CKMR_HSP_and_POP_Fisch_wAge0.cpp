   ...
   for(j=0;j<=lage;j++){  
    if((j+age_diff_hsp(i))<=lage){          //If we are not in the plus group
     if(born_year_old_hsp(i)>0){                //if we're not in unfished years
      HSP_prob_aa(i,j) = ( (N(born_year_old_hsp(i)-1,j)*Mat(j)*Waa(j)) / spbiomass(born_year_old_hsp(i)-1) ) * surv_prob(i,j)  * (4 * Mat(int(j)+age_diff_hsp(i))*Waa(int(j)+age_diff_hsp(i)) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));
	 }
	 if(born_year_old_hsp(i)<1){                //if we are in unfished years
	  if(born_year_old_hsp(i)+age_diff_hsp(i)>0){   //if second born is not in unfished years
       HSP_prob_aa(i,j) = ( (N0_age(j)*Mat(j)*Waa(j)) / SSB0 ) * surv_prob(i,j)  * (4 * Mat(int(j)+age_diff_hsp(i))*Waa(int(j)+age_diff_hsp(i)) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));
	  }
	  if(born_year_old_hsp(i)+age_diff_hsp(i)<1){   //if second born is in unfished years
       HSP_prob_aa(i,j) = ( (N0_age(j)*Mat(j)*Waa(j)) / SSB0 ) * surv_prob(i,j)  * (4 * Mat(int(j)+age_diff_hsp(i))*Waa(int(j)+age_diff_hsp(i)) / SSB0);
 	  }}
    }
	
    if((j+age_diff_hsp(i))>lage){            //If we are in the plus group
     if(born_year_old_hsp(i)>0){                //if we're not in unfished years
      HSP_prob_aa(i,j) = ( (N(born_year_old_hsp(i)-1,j)*Mat(j)*Waa(j)) / spbiomass(born_year_old_hsp(i)-1) ) * surv_prob(i,j)  * (4 * Mat(lage)*Waa(lage) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));	
	 }
	 if(born_year_old_hsp(i)<1){                //if we are in unfished years
	  if(born_year_old_hsp(i)+age_diff_hsp(i)>0){   //if second born is not in unfished years
       HSP_prob_aa(i,j) = ( (N0_age(j)*Mat(j)*Waa(j)) / SSB0 ) * surv_prob(i,j)  * (4 * Mat(lage)*Waa(lage) / spbiomass(born_year_old_hsp(i)+age_diff_hsp(i)-1));	
	  }
  	  if(born_year_old_hsp(i)+age_diff_hsp(i)<1){   //if second born is in unfished years
...
/////////////////////////
  ...
  //So the exp reproductive output of the parent in the year of offsprings birth / total reprod output that year
     if(born_year_young_pop(i)>0){
	  POP_prob(i) = 4*(Mat(age_diff_pop(i)-1)*Waa(age_diff_pop(i)-1) / spbiomass(born_year_young_pop(i)-1));
	 }    
	 if(born_year_young_pop(i)<1){
	  POP_prob(i) = 4*(Mat(age_diff_pop(i)-1)*Waa(age_diff_pop(i)-1) / SSB0);
	 }
  }
